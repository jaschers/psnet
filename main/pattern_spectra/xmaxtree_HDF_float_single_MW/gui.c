...
GUIInfo *GUIInfoCreate(double *diatompgm, double *shapepgm, long width, long height, MaxTree tree, int filter, int k,
...
void HandleDiatomEvents(GUIInfo *guiinfo, TLEvent *event)
{
   ...
   switch(event->Type)
   {
      ...
      case TLET_Mouse: 
         ...
         if ((x>=0) && (y>=0) && (x<guiinfo->DiatomWidth) && (y<guiinfo->DiatomHeight))  sprintf(str, "Diatom (%d, %d, %f)", x,y,guiinfo->DiatomPGM[y*(guiinfo->DiatomWidth)+x]);
...
int GUIShow(double *img, double *shape, long width, long height, MaxTree tree, int filter, int k, int mingreylevel, 
	    int numattrs,
