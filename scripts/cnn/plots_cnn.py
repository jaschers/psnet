...
input_cnn = np.array(["iact_images", "pattern_spectra"])
label = np.array(["CTA images", "pattern spectra"])
sigma = [[]] * len(input_cnn)

for c in range(len(input_cnn)):
    if input_cnn[c] == "iact_images":
        filename_output = f'dm-finder/cnn/{input_cnn[c]}/output/{image_type}/test_set_energy.csv' # 'test_set_energy_a_{a[0]}_{a[1]}__dl_{dl[0]}_{dl[1]}__dh_{dh[0]}_{dh[1]}__m_{m[0]}_{m[1]}__n_{n[0]}_{n[1]}__f_{f}.csv'
    ...
    # plt.legend()
    plt.savefig(path + "energy_total_histogram.png")
    ...
    plt.tight_layout()
    plt.savefig(path + f"energy_binned_histogram.png", dpi = 250)
    plt.close()
    ...
    ax[3].set_ylabel("Number of events", fontsize = 6)
    plt.tight_layout()
    plt.savefig(path + f"energy_binned_histogram_corrected.png", dpi = 250)
    plt.close()
...
plt.figure()
plt.grid(alpha = 0.2)
plt.errorbar(bins_central[skip:-2], sigma[0][skip:-2], xerr = (bins[skip:-2-1] - bins_central[skip:-2], bins_central[skip:-2] - bins[skip+1:-2]), linestyle = "", capsize = 3.0, marker = ".", label = "CTA images")
plt.errorbar(bins_central[skip:-2], sigma[1][skip:-2], xerr = (bins[skip:-2-1] - bins_central[skip:-2], bins_central[skip:-2] - bins[skip+1:-2]), linestyle = "", capsize = 3.0, marker = ".", label = "pattern spectra")
...
plt.legend()
plt.tight_layout()
plt.savefig(path + "energy_resolution.png", dpi = 250)
plt.close()
