...
losses = model.evaluate([X1_test, X2_test], Y_test, batch_size=128, verbose=0)
print("%.5e (energy)" % losses)

# predict output for test set and undo feature scaling
yp = model.predict([X1_test, X2_test], batch_size=128)
...
pd.DataFrame(table_output).to_csv(f'dm-finder/cnn/pattern_spectra/output/{image_type}/test_set_energy_2inputs_a_{a[0]}_{a[1]}__dl_{dl[0]}_{dl[1]}__dh_{dh[0]}_{dh[1]}__m_{m[0]}_{m[1]}__n_{n[0]}_{n[1]}__f_{f}.csv', index = None, header = ["log10(E_true / GeV)", "log10(E_rec / GeV)"])
