...
for r in range(len(run)): #len(run)
    ...
    for i in tqdm(range(len(table))): #len(table)
        ...
        command = "./dm-finder/scripts/pattern_spectra/xmaxtree/xmaxtree" + f" dm-finder/data/{particle_type}/images/{image_type}/" + run_filename + "/obs_id_" + f"{table['obs_id'][i]}" + "/pgm/" + "obs_id_" + f"{table['obs_id'][i]}" + "__" "event_id_" + f"{table['event_id'][i]}.pgm" f" a {a[0]}, {a[1]} dl {dl[0]}, {dl[1]} dh {dh[0]}, {dh[1]} m {m[0]}, {m[1]} n {n[0]}, {n[1]} f {f} nogui e " + path_mat + filename + " &> /dev/null"
        ...
        if r == 0 and i < 50:
            ...
            plt.xlabel("(moment of inertia) / area$^2$", labelpad = 20, fontsize = 12)
            plt.ylabel("area", labelpad = 20, fontsize = 12)
            cbar = plt.colorbar()
            cbar.set_label(label = "log$_{10}$(flux)", fontsize = 12)
            # plt.axis('off')
            plt.xticks([], [])
            plt.yticks([], [])
            plt.tight_layout()
