...
TLImage *TLCreateImage(TLWindow *window, double *data, long width, long height)
{
   TLImage *image;
   long p, x, y;
   ...
   for (y=0; y<height; y++)
   {
      ...
      {
         p = y*width+x;
         XPutPixel(image->Image, x, y, window->Display->Colors[(int) data[p]].pixel);
      }
   }
   return(image);
} /* TLCreateImage */
...
void TLRefreshImage(TLWindow *window, TLImage *image, double *data)
{
   long p, x, y;
