...
for r in range(len(run)): #len(run)
    ...
    for i in tqdm(range(len(complete_table_tel_combined))): # len(complete_table_tel_combined)
        ...
        GetEventImageBasic(image, clean_image = True, savefig = f"dm-finder/data/{particle_type}/images/{image_type}/{input_filename}/obs_id_{complete_table_tel_combined['obs_id'][i]}/tif/obs_id_{complete_table_tel_combined['obs_id'][i]}__event_id_{complete_table_tel_combined['event_id'][i]}.tif", colorbar = False, cmap = "Greys_r")
